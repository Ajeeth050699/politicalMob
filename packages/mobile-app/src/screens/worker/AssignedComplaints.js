import { complaintCategoryT, literalT } from "../../i18n/runtimeTamil";import { useTranslation } from 'react-i18next';
import React, { useEffect, useState } from 'react';
import {
  View, Text, FlatList, TouchableOpacity, StyleSheet, ScrollView,
  RefreshControl, ActivityIndicator, Platform, StatusBar, Modal } from
'react-native';
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onCancel}>
      <View style={m.overlay}>
        <View style={m.card}>
          ...
          <Text style={m.title}>{literalT("Accept Complaint?")}</Text>
          <Text style={m.desc}>{literalT("Once you accept, this complaint will be locked to you.")}
            {'\n'}{literalT("Other agents cannot work on it.")}

          </Text>
          {complaint &&
          <View style={m.info}>
              <Text style={m.infoTxt}>📋 {complaintCategoryT(complaint.category, complaint.customCategory)}</Text>
          ...
          <View style={m.btnRow}>
            <TouchableOpacity style={m.cancelBtn} onPress={onCancel} disabled={loading}>
              <Text style={m.cancelTxt}>{literalT("Cancel")}</Text>
            </TouchableOpacity>
            <TouchableOpacity style={m.acceptBtn} onPress={onConfirm} disabled={loading} activeOpacity={0.85}>
              <LinearGradient colors={[T.maroon, T.maroonL]} style={m.acceptGrad}>
                ...
                <Text style={m.acceptTxt}>{literalT("✅ Accept")}</Text>
                }
              </LinearGradient>
            </TouchableOpacity>
          </View>
    ...
    return (
      <TouchableOpacity
        ...
        <View style={s.metaRow}>
          ...
          {c.fallbackUsed && <Text style={[s.metaChip, { backgroundColor: '#fef3c7', borderColor: '#f59e0b' }]}>{literalT("🔄 Fallback")}</Text>}
          {c.escalatedToAdmin && <Text style={[s.metaChip, { backgroundColor: '#fee2e2', borderColor: T.red }]}>{literalT("⚠️ Escalated")}</Text>}
        </View>

        {/* Locked by another agent */}
        {isLocked &&
        <View style={s.lockedBanner}>
            <Text style={s.lockedTxt}>{literalT("🔒 Accepted by")}{c.assignedWorker}{literalT("— view only")}</Text>
          </View>
        }
        ...
        <View style={s.actions}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.85}>
                ...
                <LinearGradient colors={[T.maroon, T.maroonL]} style={s.acceptBtnGrad}>
                  <Text style={s.acceptBtnTxt}>{literalT("🤝 Accept Complaint")}</Text>
                </LinearGradient>
              </TouchableOpacity>
          }
          ...
          <View style={s.progressBtns}>
            ...
            <TouchableOpacity
              ...
              <Text style={[s.progBtnTxt, { color: '#8b5cf6' }]}>{literalT("⚙️ Start Work")}</Text>
              }
                  </TouchableOpacity>
            }
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.85}>
                    ...
                    <Text style={[s.progBtnTxt, { color: T.green }]}>{literalT("📸 Upload Proof & Complete")}</Text>
                  </TouchableOpacity>
            }
              </View>
          }
          ...
          <View style={s.completedBanner}>
                <Text style={s.completedTxt}>{literalT("🎉 Resolved · Tap to view proof")}</Text>
              </View>
          }
          </View>
        }
  ...
  return (
    <View style={s.root}>
      ...
      <LinearGradient colors={[T.maroon, T.maroonL]} style={s.header}>
        <Text style={s.headerTitle}>{literalT("Complaints")}</Text>
        <Text style={s.headerSub}>{literalT("Your thokuthi area complaints")}</Text>
      ...
      <FlatList
        ...
        <View style={s.empty}>
            ...
            <Text style={s.emptyTitle}>{literalT("No complaints found")}</Text>
...
const s = StyleSheet.create({
  ...
  statCard: { flex: 1, backgroundColor: 'rgba(255,255,255,0.13)', borderRadius: 10, paddingVertical: 8, paddingHorizontal: 6, alignItems: 'center' },
  statNum: { fontSize: 17, fontWeight: '900' },
  statLabel: { fontSize: 9, color: 'rgba(255,255,255,0.7)', marginTop: 1, fontWeight: '600' },

  filterScroll: { backgroundColor: '#fff', borderBottomWidth: 1, borderBottomColor: T.border, maxHeight: 44 },
  filterRow: { flexDirection: 'row', paddingHorizontal: 12, paddingVertical: 6, gap: 6, alignItems: 'center' },
  ...
  chip: { flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-start', minHeight: 30, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 16, borderWidth: 1, borderColor: T.border, backgroundColor: '#fff', gap: 5 },
  chipActive: { backgroundColor: T.maroon, borderColor: T.maroon },
  chipTxt: { fontSize: 11, fontWeight: '800', color: T.textL },
