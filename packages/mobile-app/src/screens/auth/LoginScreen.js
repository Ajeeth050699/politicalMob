      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={Platform.OS === 'ios' ? 0 : 24} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag" showsVerticalScrollIndicator={false}>
          <View style={s.card}>
            ...
            <Text style={s.label}>{literalT("Email or Phone Number")}</Text>
            <View style={s.inputRow}>
              <Text style={s.icon}>✉️</Text>
              <TextInput
                style={[s.input, { flex: 1 }]}
                placeholder={literalT("Enter email or phone")}
            ...
            <Text style={s.label}>{literalT("Password")}</Text>
            <View style={s.inputRow}>
              <Text style={s.icon}>🔒</Text>
              <TextInput
                style={[s.input, { flex: 1 }]}
                placeholder={literalT("Enter your password")}
                placeholderTextColor={T.textM}
                value={password}
            ...
            <TouchableOpacity onPress={() => navigation.navigate('ForgotPassword')} style={s.forgotBtn}>
              <Text style={s.forgotTxt}>{literalT("Forgot Password?")}</Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              <LinearGradient colors={[T.maroon, T.maroonL]} style={s.btnGrad}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                    ...
                    <Text style={s.btnTxt}>{literalT("Signing in...")}</Text>
                  </View> :

                <Text style={s.btnTxt}>{literalT("Sign In →")}</Text>
                }
              </LinearGradient>
            </TouchableOpacity>
            ...
            <Text style={s.footerTxt}>
              ...
              <Text style={{ color: T.maroon, fontWeight: '700' }} onPress={() => navigation.navigate('Register')}>{literalT("Create Account")}
