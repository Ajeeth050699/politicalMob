...
export default function RegisterScreen({ navigation }) {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      if (res.data?.otp) showToast(`Dev OTP: ${res.data.otp}`, 'info');else
  ...
  const handleStep3 = async () => {
    ...
    if (form.role === 'worker' && form.ward) {
      ...
      } catch {setThokuthiInfo(null);} finally
      {setLoading(false);}
    }
    setStep(4);
  };
        ...
        <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          <View style={s.card}>
            ...
            <>
                <View style={{ alignItems: 'center', marginBottom: 20 }}>
                  ...
                  <>
                        ...
                        <Text style={{ fontSize: 10, color: T.textM, marginTop: 4 }}>{literalT("Add Photo")}</Text>
                      </>
                  }
                  </TouchableOpacity>
                  {!profilePhoto && <Text style={{ fontSize: 11, color: T.textM, marginTop: 8 }}>{literalT("Profile photo is required")}</Text>}
                </View>
                ...
                <TouchableOpacity style={s.btn} onPress={handleStep1} activeOpacity={0.85}>
                  <Text style={s.btnText}>{literalT("Continue →")}</Text>
                </TouchableOpacity>
              </>
            }
            ...
            <>
              ...
              <>
                    <Text style={s.cardSub}>{literalT("Enter the 6-digit OTP sent to")}
                  {'\n'}
                    ...
                    <View style={s.resendRow}>
                      {countdown > 0 ?
                  <Text style={s.resendTimer}>{literalT("Resend in")}<Text style={{ fontWeight: '800', color: T.maroon }}>{countdown}s</Text></Text> :
                  <TouchableOpacity onPress={handleSendOtp}><Text style={s.resendLink}>{literalT("Did not receive?")}<Text style={{ color: T.maroon, fontWeight: '800' }}>{literalT("Resend OTP")}</Text></Text></TouchableOpacity>
                  }
                    </View>
                    <TouchableOpacity style={[s.btn, (otpCode.length < 6 || loading) && { opacity: 0.6 }]} onPress={handleVerifyOtp} disabled={otpCode.length < 6 || loading} activeOpacity={0.85}>
                      {loading ? <ActivityIndicator color="#fff" /> : <Text style={s.btnText}>{literalT("Verify OTP ✓")}</Text>}
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => {setVerifiedPhone('');setOtpCode('');}} style={s.changePh}>
                      <Text style={s.changePhTxt}>{literalT("← Change phone number")}</Text>
                    </TouchableOpacity>
                  </>
              }
              </>
            ...
            {step === 3 &&
            <>
                <Text style={fs.label}>{literalT("District *")}</Text>
                ...
                <Text style={fs.label}>{literalT("Thokuthi *")}</Text>
                <View style={s.pickerWrap}>
                  <Picker
                  selectedValue={form.ward}
                  onValueChange={(value) => setForm((f) => ({ ...f, ward: value }))}>
                  
                    <Picker.Item label={literalT("Select your Thokuthi")} value="" />
                    {wards.map((w) =>
                  <Picker.Item key={w.id} label={`${w.id}. ${w.name}`} value={w.name} />
                  )}
                  </Picker>
                </View>
                <Text style={fs.hint}>{literalT("Complaints are automatically routed to workers assigned to this Thokuthi and ward number.")}</Text>
                <Field label={literalT("Ward Number *")} icon="🏠" value={form.wardNo} onChange={set('wardNo')} placeholder={literalT("Enter ward number")} keyboard="numeric" />
                <Field label={literalT("Pincode *")} icon="📮" value={form.pincode} onChange={set('pincode')} placeholder={literalT("6-digit pincode")} keyboard="numeric" hint="Used for fallback complaint routing" />
                <Field label={literalT("Address *")} icon="🏘️" value={form.address} onChange={set('address')} placeholder={literalT("Door no, street, area")} />
                {form.role === 'worker' &&
              <Field
                label={literalT("Work Category *")}
                icon="#"
                value={form.workCategory}
                onChange={set('workCategory')}
                placeholder={literalT("e.g. Electrician, Plumber")}
              ...
              <View style={[s.infoCard, { backgroundColor: thokuthiInfo.workerCount > 0 ? '#FEF3C7' : '#DCFCE7' }]}>
                ...
                <TouchableOpacity style={[s.btn, (!form.district || loading) && { opacity: 0.6 }]} onPress={handleStep3} disabled={!form.district || loading} activeOpacity={0.85}>
                  {loading ? <ActivityIndicator color="#fff" /> : <Text style={s.btnText}>{literalT("Continue →")}</Text>}
                </TouchableOpacity>
              </>
            }
            ...
            <>
              ...
              <View style={s.workerBanner}>
                    ...
                    <Text style={{ flex: 1, fontSize: 13, color: '#1e40af', lineHeight: 19 }}>{literalT("As a")}
                  {form.role}{literalT(", you will receive new complaints from")}<Text style={{ fontWeight: '800' }}>{form.ward || form.district}</Text>{literalT(". First to accept a complaint gets it locked to you.")}
                </Text>
                  </View>
              }
                <TouchableOpacity style={[s.btn, loading && { opacity: 0.7 }]} onPress={handleRegister} disabled={loading} activeOpacity={0.85}>
                  {loading ? <ActivityIndicator color="#fff" /> : <Text style={s.btnText}>{literalT("✅ Create Account")}</Text>}
                </TouchableOpacity>
              </>
            }

            <Text style={s.footerText}>{literalT("Already have an account?")}
              {' '}
              <Text style={{ color: T.maroon, fontWeight: '700' }} onPress={() => navigation.navigate('Login')}>{literalT("Sign In")}</Text>
            </Text>
          </View>
        </ScrollView>
