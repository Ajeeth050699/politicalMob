import { literalT } from "../../i18n/runtimeTamil";import React, { useState, useRef, useEffect } from 'react';
import {
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.topBg}>
        ...
        <Text style={s.appName}>{literalT("Verify Your Phone")}</Text>
        <Text style={s.tagline}>{literalT("Check your SMS messages for a verification code.")}</Text>
      </View>
      ...
      <KeyboardAvoidingView
        ...
        <ScrollView
          ...
          <View style={s.card}>
            <Text style={s.cardTitle}>{literalT("Enter OTP")}</Text>
            ...
            <View style={s.resendRow}>
              {countdown > 0 ?
              <Text style={s.resendTimer}>{literalT("Resend OTP in")}
                {' '}
              ...
              <TouchableOpacity onPress={handleResend}>
                  <Text style={s.resendLink}>
                    {"Didn't receive? "}
                    <Text style={{ color: T.maroon, fontWeight: '800' }}>{literalT("Resend OTP")}</Text>
                  </Text>
                </TouchableOpacity>
              }
            </View>
            ...
            <TouchableOpacity
              ...
              <Text style={s.btnText}>{literalT("Verify Phone ✓")}</Text>
              }
            </TouchableOpacity>

             <Text style={s.footerText}>{literalT("A verification link has been sent to your email address. Please verify your email to complete the registration.")}
