...
export default function AdminWorkers({ navigation }) {
      ...
      <TouchableOpacity
        ...
        <View style={s.statsContainer}>
          <View style={s.statBox}>
            ...
            <Text style={s.statLabel}>{literalT("Resolved")}</Text>
          </View>
          <View style={s.statBox}>
            <Text style={[s.statNumber, { color: T.amber }]}>{w.pending || 0}</Text>
            <Text style={s.statLabel}>{literalT("Pending")}</Text>
          </View>
          <View style={s.statBox}>
            <Text style={[s.statNumber, { color: T.maroon }]}>{resolvedPct}%</Text>
            <Text style={s.statLabel}>{literalT("Resolution")}</Text>
          </View>
        </View>
        ...
        <View style={s.progressContainer}>
          ...
          <Text style={s.progressText}>{resolvedPct}{literalT("% complete")}</Text>
        </View>
  ...
  if (loading) return (
    <View style={s.center}>
      ...
      <Text style={{ color: T.textM, marginTop: 10 }}>{literalT("Loading workers...")}</Text>
    </View>);
  ...
  return (
    <View style={s.root}>
      ...
      <LinearGradient colors={[T.maroon, T.maroonL]} style={s.header}>
        ...
        <Text style={s.headerTitle}>{literalT("👷 Workers Management")}</Text>
        <Text style={s.headerSub}>{literalT("View and manage field workers")}</Text>
      </LinearGradient>
      ...
      <View style={s.searchContainer}>
        <TextInput
          style={s.searchInput}
          placeholder={literalT("🔍 Search by name, email, category, thokuthi...")}
      ...
      <View style={s.actionBar}>
        <Text style={s.resultsText}>{literalT("Showing")}
          {filtered.length}{literalT("worker")}{filtered.length !== 1 ? 's' : ''}
        </Text>
      ...
      <FlatList
        ...
        <View style={s.empty}>
            ...
            <Text style={s.emptyTxt}>{literalT("No workers found")}</Text>
            <Text style={s.emptySubTxt}>{literalT("Try adjusting your filters")}</Text>
          </View>
        }
            ...
            <View style={s.modal}>
              ...
              <ScrollView style={s.modalContent} showsVerticalScrollIndicator={false}>
                ...
                <View style={s.modalSection}>
                  <Text style={s.modalSectionTitle}>{literalT("📞 Contact Information")}</Text>
                  <View style={s.modalRow}>
                    <Text style={s.modalRowLabel}>{literalT("Email:")}</Text>
                    <Text style={s.modalRowValue}>{selectedWorker.email || 'N/A'}</Text>
                  </View>
                  <View style={s.modalRow}>
                    <Text style={s.modalRowLabel}>{literalT("Phone:")}</Text>
                ...
                {/* Location Info */}
                <View style={s.modalSection}>
                  <Text style={s.modalSectionTitle}>{literalT("📍 Location")}</Text>
                  <View style={s.modalRow}>
                    <Text style={s.modalRowLabel}>{literalT("Thokuthi:")}</Text>
                    <Text style={s.modalRowValue}>{selectedWorker.thokuthi || 'N/A'}</Text>
                  </View>
                  <View style={s.modalRow}>
                    <Text style={s.modalRowLabel}>{literalT("Ward No:")}</Text>
                    <Text style={s.modalRowValue}>{selectedWorker.wardNo || 'N/A'}</Text>
                  </View>
                  <View style={s.modalRow}>
                    <Text style={s.modalRowLabel}>{literalT("District:")}</Text>
                    <Text style={s.modalRowValue}>{selectedWorker.district || 'N/A'}</Text>
                  </View>
                  <View style={s.modalRow}>
                    <Text style={s.modalRowLabel}>{literalT("Pincode:")}</Text>
                ...
                {/* Performance */}
                <View style={s.modalSection}>
                  <Text style={s.modalSectionTitle}>{literalT("📊 Performance")}</Text>
                  <View style={s.performanceGrid}>
                    <View style={s.performanceBox}>
                      ...
                      <Text style={s.performanceLabel}>{literalT("Resolved")}</Text>
                    </View>
                    <View style={s.performanceBox}>
                      <Text style={[s.performanceNumber, { color: T.amber }]}>{selectedWorker.pending || 0}</Text>
                      <Text style={s.performanceLabel}>{literalT("Pending")}</Text>
                    </View>
                  ...
                  <View style={s.performanceBox}>
                        ...
                        <Text style={s.performanceLabel}>{literalT("Resolution")}</Text>
                      </View>
                  }
                  </View>
                </View>
        ...
        <View style={s.exportOverlay}>
          <View style={s.exportSheet}>
            <View style={s.exportHeader}>
              <Text style={s.exportTitle}>{literalT("Download workers")}</Text>
