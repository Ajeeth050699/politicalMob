...
import {
  ...
  TouchableOpacity, StatusBar, Image, Modal, Dimensions } from
'react-native';
...
export default function ComplaintDetailAdmin({ route, navigation }) {
  ...
  if (loading) return (
    <View style={s.center}>
      ...
      <Text style={{ color: T.textM, marginTop: 10 }}>{literalT("Loading complaint details...")}</Text>
    </View>);
  ...
  const detailRows = [
  { label: 'Category', value: complaintCategoryT(complaint.category, complaint.customCategory), icon: '🏷️' },
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={s.workerCard}>
          <View style={s.workerHeader}>
            <Text style={s.workerTitle}>{literalT("👷 Worker Assignment")}</Text>
          ...
          <View style={s.workerInfo}>
            ...
            <>
                ...
                <Text style={s.workerDetail}>{literalT("Status:")}{complaint.status}</Text>
                {complaint.acceptedAt &&
              <Text style={s.workerDetail}>{literalT("Accepted on:")}
                {new Date(complaint.acceptedAt).toLocaleDateString('en-IN')}
                  </Text>
              }
              </> :

            <>
                <Text style={s.noWorkerText}>{literalT("No worker has accepted this complaint yet")}</Text>
                <Text style={s.noWorkerSubText}>{literalT("System is searching for available workers...")}</Text>
              </>
            }
          </View>
          ...
          <View style={s.assignBox}>
              ...
              <View style={s.assignPickerWrap}>
                ...
                style={s.assignPicker}>
                  ...
                  <Picker.Item label={literalT("Select worker")} value="" />
                  {workers.map((w) =>
                <Picker.Item
                  key={w.id}
          ...
          <View style={s.resolvedBanner}>
              <Text style={s.resolvedText}>{literalT("✅ This complaint has been resolved")}</Text>
            </View>
          }
        </View>
        ...
        {/* ── Details Section ── */}
        <View style={s.body}>
          <Text style={s.sectionTitle}>{literalT("📋 Complaint Details")}</Text>
          ...
          <View style={s.attachSection}>
              <View style={s.attachHeader}>
                <Text style={s.attachTitle}>{literalT("📎 Proof Attachments (")}{attachments.length})</Text>
              </View>
          ...
          <View style={s.noAttachments}>
              <Text style={s.noAttachmentsText}>{literalT("📭 No attachments provided")}</Text>
            </View>
          }
        </View>
      </ScrollView>
