...
export default function ComplaintDetail({ route, navigation }) {
  ...
  if (loading) return (
    <View style={s.center}><ActivityIndicator color={T.maroon} size="large" /><Text style={{ color: T.textM, marginTop: 10 }}>{literalT("Loading...")}</Text></View>);
    ...
    <View style={s.root}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <LinearGradient colors={[T.maroon, T.maroonL]} style={s.header}>
          ...
          <Text style={s.catTitle}>{complaintCategoryT(complaint.category, complaint.customCategory)}</Text>
          <View style={s.badgeRow}>
            ...
            <View style={[s.priorityBadge, { backgroundColor: pc + '30' }]}>
              <Text style={[s.priorityTxt, { color: pc }]}>{(complaint.priority || 'medium').toUpperCase()}{literalT("PRIORITY")}</Text>
            </View>
          </View>
        </LinearGradient>
        ...
        <View style={s.body}>
          ...
          <View style={s.attachSection}>
              <View style={s.attachHeader}>
                <Text style={s.attachTitle}>{literalT("📎 Proof Attachments")}</Text>
                <View style={[s.attachCount, { backgroundColor: T.maroon }]}>
                  <Text style={s.attachCountTxt}>{attachments.length}</Text>
                </View>
              </View>
              <Text style={s.attachSub}>{literalT("Tap image to view full screen · Tap video link to open")}</Text>
              ...
              <TouchableOpacity
                ...
                <View style={[s.attachImg, s.videoPlaceholder]}>
                        ...
                        <Text style={s.videoTxt}>{literalT("Video")}</Text>
                        <Text style={s.videoOpen}>{literalT("Tap to Open")}</Text>
                      </View>
                }
          ...
          <View style={s.proofCard}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={s.proofTitle}>{literalT("Resolution Proof Uploaded")}</Text>
                <Text style={s.proofSub}>{literalT("Worker has submitted proof of resolution")}
              ...
              <TouchableOpacity onPress={() => {setViewerUri(complaint.proofPhoto);setViewerVisible(true);}}>
                <View style={s.proofBadge}><Text style={s.proofBadgeTxt}>{literalT("View")}</Text></View>
              </TouchableOpacity>
            </View>
          }
          ...
          <View style={s.proofCard}>
              <Text style={{ fontSize: 24 }}>🎥</Text>
              <View style={{ flex: 1 }}>
                <Text style={s.proofTitle}>{literalT("Resolution Video Uploaded")}</Text>
                <Text style={s.proofSub}>{literalT("Worker has submitted video proof")}</Text>
              </View>
              <TouchableOpacity onPress={() => Linking.openURL(complaint.proofVideo).catch(() => {})}>
                <View style={s.proofBadge}><Text style={s.proofBadgeTxt}>{literalT("Open")}</Text></View>
              </TouchableOpacity>
            </View>
          }
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.85}>
              ...
              <Text style={s.outlineBtnTxt}>{literalT("❌ Reject Complaint")}</Text>
            </TouchableOpacity>
          }
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.85}>
              ...
              <Text style={s.outlineBtnTxt}>{literalT("🗑️ Revoke Complaint")}</Text>
            </TouchableOpacity>
          }
          ...
          <View style={s.infoCard}>
              ...
              <Text style={s.infoTxt}>{literalT("Your complaint is pending worker assignment. A worker from your thokuthi will be assigned shortly.")}</Text>
            </View>
          }
        </View>
      </ScrollView>
      ...
      <Modal visible={rejectModal} transparent animationType="slide" onRequestClose={() => setRejectModal(false)}>
        <View style={s.modalOverlay}>
          <View style={s.modalCard}>
            <Text style={s.modalTitle}>{literalT("Reject Complaint")}</Text>
            <Text style={s.modalDesc}>{literalT("Please provide a reason for rejecting this complaint. It will be reassigned.")}</Text>
            <View style={s.inputWrap}>
              <Text style={{ fontSize: 16, marginRight: 8 }}>💬</Text>
              <TextInput
                style={{ flex: 1, paddingVertical: 12, fontSize: 15, color: T.text }}
                placeholder={literalT("Enter rejection reason...")}
            ...
            <View style={s.modalActions}>
              <TouchableOpacity style={s.modalCancelBtn} onPress={() => {setRejectModal(false);setReasonText('');}}>
                <Text style={s.modalCancelTxt}>{literalT("Cancel")}</Text>
              </TouchableOpacity>
      ...
      <Modal visible={revokeModal} transparent animationType="slide" onRequestClose={() => setRevokeModal(false)}>
        <View style={s.modalOverlay}>
          <View style={s.modalCard}>
            <Text style={s.modalTitle}>{literalT("Revoke Complaint")}</Text>
            <Text style={s.modalDesc}>{literalT("Are you sure you want to cancel this complaint?")}</Text>
            ...
            <View style={s.inputWrap}>
              <TextInput
                style={{ flex: 1, paddingVertical: 12, fontSize: 15, color: T.text }}
                placeholder={literalT("Optional explanation...")}
                placeholderTextColor={T.textM}
                value={reasonText}
                onChangeText={setReasonText} />
