import { literalT } from "../../i18n/runtimeTamil";import React, { useRef, useEffect } from 'react';
import {
  View, Text, ScrollView, TouchableOpacity, StyleSheet,
  Platform, StatusBar, Animated, Dimensions, Share } from
'react-native';
...
export default function NewsDetailScreen({ route, navigation }) {
  ...
  if (!news) {
    return (
      <View style={d.root}>
        <Text style={{ padding: 20, color: T.textM }}>{literalT("News not found.")}</Text>
      </View>);
    ...
    <View style={d.root}>
      ...
      <Animated.ScrollView
        ...
        <Animated.View style={[d.bodyCard, {
          ...
          <View style={d.metaStrip}>
            ...
            <View style={d.metaItem}>
              ...
              <Text style={d.metaMd}>{news.level}{literalT("Level")}</Text>
            </View>
            <View style={d.metaDivider} />
            <View style={d.metaItem}>
              <Text style={d.metaIcon}>⏱</Text>
              <Text style={d.metaMd}>{literalT("3 min read")}</Text>
            </View>
          </View>
          ...
          <View style={d.tagsSection}>
              <Text style={d.tagsSectionTitle}>{literalT("Tags")}</Text>
              <View style={d.tagRow}>
                {news.tags.map((t) =>
          ...
          <TouchableOpacity style={[d.shareCta, { borderColor: meta.color + '30', backgroundColor: meta.bg }]} onPress={handleShare} activeOpacity={0.85}>
            <Text style={[d.shareCtaTxt, { color: meta.color }]}>{literalT("📤 Share this news")}</Text>
          </TouchableOpacity>
        </Animated.View>
        ...
        <View style={d.relSection}>
            <Text style={d.relHeader}>{literalT("More News")}</Text>
            {relatedNews.map((n, i) =>
          <RelatedCard
            key={n.id}
            n={n}
