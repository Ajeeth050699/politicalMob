...
export default function MyComplaintsScreen({ navigation }) {
        ...
        <View style={s.cardContent}>
          ...
          <View style={s.cardTop}>
            <View style={s.categoryRow}>
              ...
              <Text style={s.categoryText} numberOfLines={1}>{complaintCategoryT(c.category, c.customCategory)}</Text>
            </View>
  ...
  if (loading) return (
    <View style={s.center}>
      ...
      <Text style={{ color: T.textM, marginTop: 12 }}>{literalT("Loading complaints...")}</Text>
    </View>);
  ...
  return (
    <View style={s.root}>
      ...
      <LinearGradient colors={[T.maroon, T.maroonL]} style={s.header}>
        <Text style={s.headerTitle}>{literalT("My Complaints")}</Text>
        <Text style={s.headerSub}>{literalT("Track all your submitted issues")}</Text>
        ...
        <View style={s.statsRow}>
          ...
          <View key={label} style={s.statCard}>
              ...
              <Text style={s.statLabel}>{literalT(label)}</Text>
            </View>
          )}
        </View>
      </LinearGradient>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}>
            ...
            <Text style={[s.filterText, filter === f && { color: '#fff', fontWeight: '700' }]}>
              {f === 'ALL' ? literalT('All') : f === 'IN PROGRESS' ? literalT('In Progress') : literalT(f)}
            </Text>
          </TouchableOpacity>
        )}
      </ScrollView>
      ...
      <FlatList
        ...
        <View style={s.empty}>
            ...
            <Text style={s.emptyTitle}>{literalT("No complaints yet")}</Text>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.85}>
                ...
                <Text style={s.addBtnText}>{literalT("+ Report an Issue")}</Text>
              </TouchableOpacity>
          }
          </View>
        } />
