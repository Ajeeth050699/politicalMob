...
export default function AdminForgotPassword() {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      if (ph) setPhone(ph);
      if (devOtp) showToast(`Dev OTP: ${devOtp} (sent to registered phone)`, "info");else
      showToast("OTP sent to your registered phone number.", "success");
      setStep(2);
    } catch (err) {
      const s = err?.response?.status;
      if (s === 404) showToast("No account found with this email address.");else
    ...
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "'Source Sans 3',sans-serif" }}>
      ...
      <div style={{ width: "100%", maxWidth: 420, animation: "fadeIn 0.5s ease" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <h1 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, fontSize: 22, color: C.text }}>{literalT("People Connect")}</h1>
          <p style={{ fontSize: 12, color: C.muted, marginTop: 3 }}>{literalT("Tamil Nadu Government Portal")}</p>
        </div>
        ...
        <div style={{ background: "#fff", borderRadius: 24, padding: "36px 32px", boxShadow: "0 8px 40px rgba(0,0,0,0.08)", border: `1px solid ${C.border}` }}>
          ...
          <div>
              <div style={{ marginBottom: 20 }}>
                <label style={labelStyle}>{literalT("Registered Email")}</label>
              ...
              <button onClick={handleSendOtp} disabled={loading} className="sbtn" style={btnStyle}>
                {loading ? <span style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}><span style={{ width: 14, height: 14, border: "2px solid rgba(255,255,255,0.3)", borderTopColor: "#fff", borderRadius: "50%", display: "inline-block", animation: "spin 0.75s linear infinite" }} />{literalT("Sending OTP...")}</span> : "Send OTP →"}
              </button>
            </div>
          }
          ...
          {step === 2 &&
          <div>
              <p style={{ fontSize: 13, color: C.muted, marginBottom: 20, lineHeight: 1.6 }}>{literalT("OTP sent to the phone number registered with")}<strong style={{ color: C.text }}>{email}</strong></p>
              <div style={{ marginBottom: 20 }}>
                <label style={labelStyle}>{literalT("6-Digit OTP")}</label>
              ...
              <button onClick={() => {setStep(1);setOtp("");setToast({ msg: "", type: "error" });}} style={{ width: "100%", padding: "10px", background: "none", border: "none", color: C.muted, fontFamily: "'Source Sans 3',sans-serif", fontSize: 13, cursor: "pointer" }}>{literalT("← Try different email")}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                <label style={labelStyle}>{literalT("New Password")}</label>
                <div style={{ position: "relative" }}>
                  <span style={{ position: "absolute", left: 13, top: "50%", transform: "translateY(-50%)", fontSize: 15 }}>🔒</span>
                  <input className="inp" type={showPw ? "text" : "password"} value={newPw} onChange={(e) => setNewPw(e.target.value)} placeholder={literalT("Minimum 6 characters")} style={{ ...inputStyle, paddingRight: 40 }} />
              ...
              <div>
                <label style={labelStyle}>{literalT("Confirm New Password")}</label>
                <div style={{ position: "relative" }}>
                  <span style={{ position: "absolute", left: 13, top: "50%", transform: "translateY(-50%)", fontSize: 15 }}>🔒</span>
                  <input className="inp" type="password" value={confirmPw} onChange={(e) => setConfirmPw(e.target.value)} placeholder={literalT("Re-enter new password")} style={inputStyle} />
                </div>
              </div>
              {/* Password strength */}
              {newPw &&
            <div>
                ...
                <div key={i} style={{ flex: 1, height: 4, borderRadius: 2, background: newPw.length >= i * 3 ? newPw.length < 6 ? "#ef4444" : newPw.length < 9 ? C.gold : "#22c55e" : C.border }} />
                  ...
                  <p style={{ fontSize: 11, color: newPw.length < 6 ? "#ef4444" : newPw.length < 9 ? C.gold : "#22c55e", fontWeight: 600 }}>
                    {newPw.length < 6 ? "Weak" : newPw.length < 9 ? "Medium" : "Strong"}{literalT("password")}
              </p>
                </div>
            }
          ...
          <p style={{ textAlign: "center", fontSize: 13, color: C.muted, marginTop: 22 }}>{literalT("Remember your password?")}
            {" "}
