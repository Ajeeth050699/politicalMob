...
export default function AdminDashboard() {
  ...
  const AddCampModal = () => {
    ...
    return (
      <Modal title={literalT("🏕️ Create Welfare Camp")} onClose={() => setModal(null)} wide>
        <form onSubmit={submit}>
          <Field label={literalT("Camp Name")} icon="🏕️">
            <input
              style={iSx}
              required
              value={f.name}
    ...
    style={{
      ...
      style={{
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          <span style={{ padding: "5px 10px", borderRadius: 50, background: "rgba(123,28,28,0.08)", color: T.maroon, fontSize: 12, fontWeight: 700 }}>{literalT("Status:")}
          {filterStatus === "ALL" ? "All" : filterStatus}
          </span>
          <span style={{ padding: "5px 10px", borderRadius: 50, background: "rgba(123,28,28,0.08)", color: T.maroon, fontSize: 12, fontWeight: 700 }}>{literalT("District:")}
  ...
  const PageSuperAdmin = () => {
    ...
    const visibleUsers = users.filter((u) => {
      ...
      const text = `${u.name || ""} ${u.email || ""} ${u.phone || ""} ${u.thokuthi || ""} ${u.district || ""}`.toLowerCase();
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 16, padding: 18, border: `1px solid ${T.border}` }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: T.text, marginBottom: 14 }}>{literalT("Create Account")}</div>
          <form onSubmit={createUser} style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(4,1fr)", gap: 12 }}>
            <input style={iSx} placeholder={literalT("Name")} value={form.name} onChange={set("name")} required />
            <input style={iSx} placeholder={literalT("Email")} value={form.email} onChange={set("email")} required />
            <input style={iSx} placeholder={literalT("Mobile")} value={form.phone} onChange={set("phone")} />
            <input style={iSx} placeholder={literalT("Password")} value={form.password} onChange={set("password")} required />
            ...
            <input style={iSx} placeholder={literalT("Thokuthi number")} value={form.thokuthi} onChange={set("thokuthi")} />
            <DistrictSelect value={form.district} onChange={set("district")} />
            <input style={iSx} placeholder={literalT("Pincode")} value={form.pincode} onChange={set("pincode")} />
            <input style={{ ...iSx, gridColumn: isMobile ? "auto" : "span 3" }} placeholder={literalT("Address / area")} value={form.address} onChange={set("address")} />
            <button type="submit" style={{ border: "none", borderRadius: 12, background: T.maroon, color: "#fff", fontWeight: 800, cursor: "pointer" }}>{literalT("Create")}</button>
          </form>
        </div>
        <div style={{ background: "#fff", borderRadius: 16, padding: 18, border: `1px solid ${T.border}` }}>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 14 }}>
            <input style={{ ...iSx, flex: 1, minWidth: 220 }} placeholder={literalT("Search users...")} value={q} onChange={(e) => setQ(e.target.value)} />
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 860 }}>
              ...
              <tbody>{visibleUsers.map((u) =>
                <tr key={u.id} style={{ borderBottom: `1px solid ${T.border}` }}>
                  ...
                  <td style={{ padding: 12 }}>{u.thokuthi || "-"}</td>
                  ...
                  <td style={{ padding: 12 }}>
                    ...
                    <button onClick={() => deleteUser(u)} disabled={u.id === currentUser._id} style={{ padding: "7px 10px", borderRadius: 8, border: `1px solid ${T.red}40`, background: `${T.red}10`, color: T.red, cursor: "pointer", opacity: u.id === currentUser._id ? 0.5 : 1 }}>{literalT("Delete")}</button>
                  </td>
                </tr>
                )}</tbody>
            </table>
