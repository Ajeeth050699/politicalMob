import { literalT } from "./i18n/runtimeTamil";import { useEffect, useRef, useState } from "react";
    ...
    <div style={{ fontFamily: "'Source Sans 3',sans-serif", background: "#fff", overflowX: "hidden" }}>
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }} onClick={() => window.scrollTo(0, 0)}>
          ...
          <div>
            <div style={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, fontSize: 15, color: "#fff", lineHeight: 1 }}>{literalT("People Connect")}</div>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.55)", letterSpacing: 1.2, textTransform: "uppercase" }}>{literalT("Tamil Nadu")}</div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button className="btn-outline" onClick={() => navigate("/login")} style={{ padding: "8px 22px", borderRadius: 50, border: "1.5px solid rgba(255,255,255,0.4)", background: "transparent", color: "#fff", fontFamily: "'Source Sans 3',sans-serif", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>{literalT("Sign In")}</button>
          <button className="btn-gold" onClick={() => navigate("/register")} style={{ padding: "8px 22px", borderRadius: 50, border: "none", background: `linear-gradient(135deg,${C.gold},${C.goldL})`, color: C.dark, fontFamily: "'Source Sans 3',sans-serif", fontWeight: 700, fontSize: 13, cursor: "pointer", boxShadow: `0 4px 16px rgba(201,152,42,0.35)` }}>{literalT("Get Started")}</button>
        </div>
      </header>
      ...
      <section style={{ minHeight: "100vh", background: `linear-gradient(145deg,${C.dark} 0%,${C.maroon} 55%,#AA3535 100%)`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "100px clamp(20px,5vw,60px) 60px", position: "relative", overflow: "hidden" }}>
        ...
        <div className="fadeUp d1" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.1)", borderRadius: 50, padding: "8px 20px", marginBottom: 28, border: "1px solid rgba(255,255,255,0.18)" }}>
          <span>🇮🇳</span>
          <span style={{ fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,0.9)", letterSpacing: 0.8 }}>{literalT("OFFICIAL GOVERNMENT OF TAMIL NADU PORTAL")}</span>
        </div>

        {/* Headline */}
        <h1 className="fadeUp d2" style={{ fontFamily: "'Playfair Display',serif", fontWeight: 900, fontSize: "clamp(40px,7vw,80px)", color: "#fff", textAlign: "center", lineHeight: 1.05, marginBottom: 10, maxWidth: 820 }}>{literalT("People Connect")}
        ...
        }}>{literalT("Tamil Nadu Public Service Platform")}</h2>
        ...
        <div className="fadeUp d4" style={{ display: "flex", gap: 14, flexWrap: "wrap", justifyContent: "center", marginBottom: 64 }}>
          <button className="btn-gold" onClick={() => navigate("/login")} style={{ padding: "15px 36px", borderRadius: 50, border: "none", background: `linear-gradient(135deg,${C.gold},${C.goldL})`, color: C.dark, fontFamily: "'Source Sans 3',sans-serif", fontWeight: 800, fontSize: 16, cursor: "pointer", boxShadow: `0 6px 24px rgba(201,152,42,0.4)` }}>{literalT("🏛️ Admin Dashboard →")}

          </button>
          <button className="btn-outline" onClick={() => navigate("/register")} style={{ padding: "15px 36px", borderRadius: 50, border: "2px solid rgba(255,255,255,0.35)", background: "rgba(255,255,255,0.08)", color: "#fff", fontFamily: "'Source Sans 3',sans-serif", fontWeight: 700, fontSize: 16, cursor: "pointer" }}>{literalT("Create Account")}
      ...
      <section style={{ background: C.bg, padding: "80px clamp(20px,5vw,60px)" }}>
        <div style={{ textAlign: "center", marginBottom: 52 }}>
          <p style={{ fontSize: 12, fontWeight: 700, color: C.maroon, letterSpacing: 2, textTransform: "uppercase", marginBottom: 10 }}>{literalT("What We Offer")}</p>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, fontSize: "clamp(26px,4vw,42px)", color: C.text, marginBottom: 14 }}>{literalT("Everything in One Place")}</h2>
          <p style={{ fontSize: 15, color: C.muted, maxWidth: 480, margin: "0 auto", lineHeight: 1.7 }}>{literalT("A unified platform connecting citizens, workers and administrators across Tamil Nadu.")}</p>
        </div>
      ...
      <section style={{ background: "#fff", padding: "80px clamp(20px,5vw,60px)" }}>
        <div style={{ textAlign: "center", marginBottom: 52 }}>
          <p style={{ fontSize: 12, fontWeight: 700, color: C.maroon, letterSpacing: 2, textTransform: "uppercase", marginBottom: 10 }}>{literalT("Simple Process")}</p>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, fontSize: "clamp(26px,4vw,42px)", color: C.text }}>{literalT("How It Works")}</h2>
        </div>
      ...
      <section style={{ background: `linear-gradient(135deg,${C.dark},${C.maroon},#AA3535)`, padding: "72px clamp(20px,5vw,60px)", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <h2 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, fontSize: "clamp(22px,4vw,38px)", color: "#fff", marginBottom: 12, position: "relative" }}>{literalT("Ready to Make Tamil Nadu Better?")}

        </h2>
        <p style={{ fontSize: 15, color: "rgba(255,255,255,0.7)", marginBottom: 36, position: "relative" }}>{literalT("Join thousands of citizens making a difference, one complaint at a time.")}

        </p>
        <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap", position: "relative" }}>
          <button className="btn-gold" onClick={() => navigate("/register")} style={{ padding: "14px 32px", borderRadius: 50, border: "none", background: `linear-gradient(135deg,${C.gold},${C.goldL})`, color: C.dark, fontFamily: "'Source Sans 3',sans-serif", fontWeight: 800, fontSize: 15, cursor: "pointer", boxShadow: `0 4px 20px rgba(201,152,42,0.4)` }}>{literalT("Get Started Free →")}

          </button>
          <button className="btn-outline" onClick={() => navigate("/login")} style={{ padding: "14px 32px", borderRadius: 50, border: "2px solid rgba(255,255,255,0.35)", background: "transparent", color: "#fff", fontFamily: "'Source Sans 3',sans-serif", fontWeight: 700, fontSize: 15, cursor: "pointer" }}>{literalT("Admin Login")}

          </button>
          <button className="btn-outline" onClick={() => window.location.href = 'http://localhost:5174'} style={{ padding: "14px 32px", borderRadius: 50, border: "2px solid rgba(255,255,255,0.35)", background: "transparent", color: "#fff", fontFamily: "'Source Sans 3',sans-serif", fontWeight: 700, fontSize: 15, cursor: "pointer" }}>{literalT("Super Admin")}
      ...
      <footer style={{ background: C.dark, padding: "28px clamp(20px,5vw,60px)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, fontSize: 13, color: "#fff" }}>{literalT("People Connect")}</div>
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)" }}>{literalT("Tamil Nadu Government Portal")}</div>
            </div>
          </div>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.35)" }}>{literalT("© 2025 Government of Tamil Nadu. All rights reserved.")}</div>
        </div>
      </footer>
    </div>);
