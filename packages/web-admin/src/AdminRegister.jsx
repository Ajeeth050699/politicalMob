import { literalT } from "./i18n/runtimeTamil";import { useState, useEffect } from "react";
...
export default function AdminRegister() {
  ...
  const [form, setForm] = useState({ name: "", email: "", phone: "", password: "", confirmPassword: "", district: "Chennai", ward: "", tamilNaduAccess: false });
  ...
  const handleStep1 = async () => {
    ...
    try {
      ...
      if (devOtp) showToast(`Dev OTP: ${devOtp}`, "info");else
  ...
  const handleStep3 = async () => {
    ...
    try {
      const { data } = await axios.post(`${API}/api/auth/register`, {
        ...
        password: form.password, role: "admin", district: form.district,
    ...
    <div style={{ minHeight: "100vh", display: "grid", gridTemplateColumns: "1fr 1fr", fontFamily: "'Source Sans 3',sans-serif" }}>
      ...
      <div style={{ background: `linear-gradient(145deg,${C.dark},${C.maroon},#AA3535)`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "48px 44px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", width: "100%", maxWidth: 320, textAlign: "center" }}>
          ...
          <h1 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 900, fontSize: 28, color: "#fff", marginBottom: 8 }}>{literalT("Join the Portal")}</h1>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", marginBottom: 40, lineHeight: 1.7 }}>{literalT("Create your People Connect admin account in 3 simple steps.")}</p>
          {[
      ...
      <div style={{ background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 32px" }}>
        <div style={{ width: "100%", maxWidth: 400, animation: "fadeIn 0.5s ease" }}>
          <Link to="/" style={{ fontSize: 13, color: C.muted, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6, marginBottom: 28 }}>{literalT("← Back to Home")}</Link>
          ...
          <div style={{ background: "#fff", borderRadius: 24, padding: "36px 32px", boxShadow: "0 8px 40px rgba(0,0,0,0.08)", border: `1px solid ${C.border}` }}>
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, fontSize: 24, color: C.text, marginBottom: 4 }}>{literalT("Create Account")}</h2>
            <p style={{ fontSize: 13, color: C.muted, marginBottom: 24 }}>{literalT("Step")}{step}{literalT("of 3 —")}{["Personal Details", "OTP Verification", "Access Area"][step - 1]}</p>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div>
                  <label style={labelStyle}>{literalT("Full Name")}</label>
                  <div style={{ position: "relative" }}><span style={{ position: "absolute", left: 13, top: "50%", transform: "translateY(-50%)", fontSize: 15 }}>👤</span>
                    <input className="inp" value={form.name} onChange={set("name")} placeholder={literalT("Your full name")} style={inputStyle} /></div>
                </div>
                <div>
                  <label style={labelStyle}>{literalT("Email Address")}</label>
                ...
                <div>
                  <label style={labelStyle}>{literalT("Phone Number")}</label>
                  <div style={{ position: "relative" }}><span style={{ position: "absolute", left: 13, top: "50%", transform: "translateY(-50%)", fontSize: 15 }}>📱</span>
                    <input className="inp" type="tel" value={form.phone} onChange={set("phone")} placeholder={literalT("10-digit mobile number")} style={inputStyle} maxLength={10} /></div>
                </div>
                <div>
                  <label style={labelStyle}>{literalT("Password")}</label>
                  <div style={{ position: "relative" }}><span style={{ position: "absolute", left: 13, top: "50%", transform: "translateY(-50%)", fontSize: 15 }}>🔒</span>
                    <input className="inp" type={showPw ? "text" : "password"} value={form.password} onChange={set("password")} placeholder={literalT("Minimum 6 characters")} style={{ ...inputStyle, paddingRight: 40 }} />
                ...
                <div>
                  <label style={labelStyle}>{literalT("Confirm Password")}</label>
                  <div style={{ position: "relative" }}><span style={{ position: "absolute", left: 13, top: "50%", transform: "translateY(-50%)", fontSize: 15 }}>🔒</span>
                    <input className="inp" type="password" value={form.confirmPassword} onChange={set("confirmPassword")} placeholder={literalT("Re-enter password")} style={inputStyle} /></div>
                </div>
                <button onClick={handleStep1} disabled={loading} className="sbtn" style={{ marginTop: 6, padding: "13px", borderRadius: 50, border: "none", background: `linear-gradient(135deg,${C.maroon},${C.light})`, color: "#fff", fontFamily: "'Source Sans 3',sans-serif", fontWeight: 700, fontSize: 15, cursor: loading ? "not-allowed" : "pointer", boxShadow: `0 4px 18px rgba(123,28,28,0.28)`, opacity: loading ? 0.75 : 1 }}>
                  {loading ? <span style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}><span style={{ width: 14, height: 14, border: "2px solid rgba(255,255,255,0.3)", borderTopColor: "#fff", borderRadius: "50%", display: "inline-block", animation: "spin 0.75s linear infinite" }} />{literalT("Sending OTP...")}</span> : "Continue →"}
                </button>
              </div>
            }
            ...
            {step === 2 &&
            <div>
                <p style={{ fontSize: 14, color: C.muted, marginBottom: 20, lineHeight: 1.6 }}>{literalT("A 6-digit OTP has been sent to")}<strong style={{ color: C.text }}>+91 {form.phone}</strong></p>
                <div style={{ marginBottom: 20 }}>
                  <label style={labelStyle}>{literalT("Enter OTP")}</label>
                  <input className="inp" value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, "").slice(0, 6))} placeholder={literalT("6-digit OTP")} maxLength={6}
                ...
                <button onClick={() => setStep(1)} style={{ width: "100%", padding: "10px", background: "none", border: "none", color: C.muted, fontFamily: "'Source Sans 3',sans-serif", fontSize: 13, cursor: "pointer" }}>{literalT("← Change phone number")}</button>
              </div>
            }
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div>
                  <label style={labelStyle}>{literalT("District")}</label>
                  <div style={{ position: "relative" }}><span style={{ position: "absolute", left: 13, top: "50%", transform: "translateY(-50%)", fontSize: 15 }}>📍</span>
                ...
                <div>
                  <label style={labelStyle}>{literalT("Thokuthi")}<span style={{ color: C.muted, fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>{literalT("(optional)")}</span></label>
                  <div style={{ position: "relative" }}><span style={{ position: "absolute", left: 13, top: "50%", transform: "translateY(-50%)", fontSize: 15 }}>🏠</span>
                    <input className="inp" value={form.ward} onChange={set("ward")} placeholder={literalT("e.g. Sivakasi")} style={inputStyle} /></div>
                </div>
                <label style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 13, color: C.text, fontWeight: 700 }}>
                  <input type="checkbox" checked={form.tamilNaduAccess} onChange={(e) => setForm((p) => ({ ...p, tamilNaduAccess: e.target.checked }))} />{literalT("Tamil Nadu overall access")}

              </label>
                <div style={{ fontSize: 12, color: C.muted, lineHeight: 1.5 }}>{literalT("Admins do not need ward number or pincode. Use Thokuthi for local access, or Tamil Nadu overall access for statewide monitoring.")}
            ...
            <p style={{ textAlign: "center", fontSize: 13, color: C.muted, marginTop: 22 }}>{literalT("Already have an account?")}
              {" "}
              <Link to="/login" style={{ color: C.maroon, fontWeight: 700, textDecoration: "none" }}>{literalT("Sign in")}</Link>
            </p>
          </div>
        </div>
      </div>
