...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (s === 401) showToast("Incorrect email or password.");else
      if (s === 404) showToast("No account found with this email.");else
      if (!err?.response) showToast("Cannot reach server. Check your connection.", "warning");else
    ...
    <div style={{ minHeight: "100vh", display: "grid", gridTemplateColumns: "1fr 1fr", fontFamily: "'Source Sans 3',sans-serif" }}>
      ...
      <div className="leftpan" style={{ background: `linear-gradient(145deg,${C.dark},${C.maroon},#AA3535)`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "48px 44px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", width: "100%", maxWidth: 340 }}>
          ...
          <h1 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 900, fontSize: 30, color: "#fff", marginBottom: 6, lineHeight: 1.15 }}>{literalT("People Connect")}</h1>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.55)", marginBottom: 40, letterSpacing: 0.5 }}>{literalT("Tamil Nadu Government Admin Portal")}</p>
          {[
      ...
      <div style={{ background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 32px" }}>
        <div style={{ width: "100%", maxWidth: 400, animation: "fadeIn 0.5s ease" }}>
          <Link to="/" style={{ fontSize: 13, color: C.muted, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6, marginBottom: 32 }} className="lnk">{literalT("← Back to Home")}
          ...
          <div style={{ background: "#fff", borderRadius: 24, padding: "36px 32px", boxShadow: "0 8px 40px rgba(0,0,0,0.08)", border: `1px solid ${C.border}` }}>
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, fontSize: 26, color: C.text, marginBottom: 4 }}>{literalT("Welcome Back")}</h2>
            <p style={{ fontSize: 14, color: C.muted, marginBottom: 28 }}>{literalT("Sign in to your admin account")}</p>
            ...
            <form onSubmit={handleSubmit} noValidate>
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#6B6B6B", marginBottom: 7, textTransform: "uppercase", letterSpacing: 0.7 }}>{literalT("Email")}</label>
              ...
              <div style={{ marginBottom: 12 }}>
                <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#6B6B6B", marginBottom: 7, textTransform: "uppercase", letterSpacing: 0.7 }}>{literalT("Password")}</label>
                <div style={{ position: "relative" }}>
                  <span style={{ position: "absolute", left: 13, top: "50%", transform: "translateY(-50%)", fontSize: 15, pointerEvents: "none" }}>🔒</span>
                  <input className="inp" type={showPw ? "text" : "password"} value={form.password} onChange={set("password")} placeholder={literalT("Enter your password")} autoComplete="current-password"
              ...
              <div style={{ textAlign: "right", marginBottom: 22 }}>
                <Link to="/forgot-password" style={{ fontSize: 13, color: C.maroon, fontWeight: 600, textDecoration: "none" }}>{literalT("Forgot Password?")}</Link>
              </div>
              ...
              <button type="submit" disabled={loading} className="sbtn" style={{ width: "100%", padding: "13px", borderRadius: 50, border: "none", background: `linear-gradient(135deg,${C.maroon},${C.light})`, color: "#fff", fontFamily: "'Source Sans 3',sans-serif", fontWeight: 700, fontSize: 15, cursor: loading ? "not-allowed" : "pointer", boxShadow: `0 4px 18px rgba(123,28,28,0.28)`, opacity: loading ? 0.75 : 1 }}>
                ...
                <span style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
                      <span style={{ width: 15, height: 15, border: "2px solid rgba(255,255,255,0.3)", borderTopColor: "#fff", borderRadius: "50%", display: "inline-block", animation: "spin 0.75s linear infinite" }} />{literalT("Signing in...")}
            ...
            <p style={{ textAlign: "center", fontSize: 13, color: C.muted, marginTop: 22 }}>{literalT("Don't have an account?")}
              {" "}
              <Link to="/register" style={{ color: C.maroon, fontWeight: 700, textDecoration: "none" }}>{literalT("Register here")}</Link>
            </p>
          </div>
          ...
          <div style={{ marginTop: 16, padding: "12px 16px", background: "rgba(123,28,28,0.06)", borderRadius: 12, border: `1px solid rgba(123,28,28,0.12)` }}>
            <p style={{ fontSize: 12, color: C.maroon, textAlign: "center", fontWeight: 600 }}>{literalT("🔒 Restricted to authorised Tamil Nadu government personnel only")}
